...
- **Webhook deliveries (BREAKING for receivers):** signatures now cover `${timestamp}.${body}` and requests carry `x-munin-timestamp`. Verify by checking the timestamp against a freshness window, then recomputing the HMAC over `timestamp + "." + rawBody`.
