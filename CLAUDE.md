...
- **Fixture data must never identify a real person.** Fixtures, changesets, PR bodies and commit messages are published surfaces — they reach tarballs, changelogs, release notes and pull request descriptions, and not all of those can be retracted later. Email addresses use a reserved domain (`.test`, `.example`, `.invalid`, or `example.com`/`example.no`); names use obvious placeholders (`Ola Nordmann`, `Kari Nordmann`, `Acme`, `Globex`); Norwegian phone numbers use a national number starting `0` or `1` (e.g. `+4712345678`), which Norway never assigns — a number starting 2-9 may be assigned to someone even if you invented it. `scripts/check-fixture-pii.mjs` enforces this in pre-commit and CI; the two allow-lists in it are the review checkpoint, so adding an entry should be a deliberate decision with a stated reason.
