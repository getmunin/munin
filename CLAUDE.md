...
| `conv` | `conv_*` | Email / SMS / WhatsApp / voice / widget channels via the channel-adapter contract (`packages/backend-core/src/modules/conv/CLAUDE.md`). |
...
- **Messaging channel** — a surface customers write to us on (email, SMS, WhatsApp, voice, widget). Implement a channel adapter in `conv` (contract: `packages/backend-core/src/modules/conv/CLAUDE.md`).
...
- **Connector** — a customer's system of record we answer questions from (Shopify, Magento, Gastroplanner, Bing Webmaster Tools, Google Search Console). Plumbing lives in the `connectors` trunk module (connection storage, credential encryption, vendor registry, `connectors_*` admin tools); typed read surfaces live in domain modules (`commerce`, `bookings`, `seo`) whose vendor adapters register into the trunk registry. A domain module is a distinct noun with its own contract (orders, bookings, search performance) — never a vendor. Connector data is live: nothing is persisted in Munin. Most domains are read-only; `bookings` also writes (availability + create/modify/cancel) and `seo` submits URLs for recrawl, both directly to the vendor.
  ...
  A domain module normalizes across engines, and where a vendor cannot do something the interface method is *optional* rather than faked: `SeoAdapter.submitUrls` exists on Bing and not on Google Search Console, and the service reports that plainly instead of pretending. Fields work the same way — `seo_inspect_url` returns nulls for what an engine doesn't expose.
  ...
  Self-hosters cannot use Munin's OAuth client, so client id and secret are per-connection config fields rather than deployment env vars. Google Search Console additionally needs `webmasters.readonly`, a sensitive scope: an org's own OAuth client works unverified for accounts it owns, but distributing one to customers requires Google app verification (CASA assessment, privacy policy, demo video).
...
- **Split read and write.** No tool that both reads and mutates, and no catch-all `method`-style param; keep `create` / `update` / `delete` as separate tools. One deliberate exception, and the verb is what marks it: a `<module>_configure_*` tool creates *or* updates in one call, keyed by an optional id (`conv_configure_email_channel`, `conv_configure_voice_sms_channel`). Only reach for it when create and update really are the same write, and never when create produces something unrepeatable — a tool that mints a one-time secret stays its own `create` (`conv_create_widget_channel`), because otherwise the return shape changes depending on whether an argument was passed, and a re-call is ambiguous about whether to rotate the secret.
...
- **Fixture data must never identify a real person.** Fixtures, changesets, commit messages and PR bodies are published surfaces — they reach tarballs, changelogs, release notes and pull request descriptions, and not all of those can be retracted later. Email addresses use a reserved domain (`.test`, `.example`, `.invalid`, or `example.com`/`example.no`); names use obvious placeholders (`Ola Nordmann`, `Kari Nordmann`, `Acme`, `Globex`); hostnames follow the same rule, because a customer's own domain names them just as surely as their address does. Phone numbers must come from a block no operator can assign: a national number starting `0` (most plans strip the trunk prefix, so `+4712345678` is safe while `+4791234567` may be someone real), NANP `555-0100`–`555-0199`, or the Ofcom drama ranges for `+44`.

  One detector in `scripts/lib/pii.mjs` backs four gates, so a rule fixed once is fixed everywhere: `scripts/check-fixture-pii.mjs` over tracked files (pre-commit + CI), `scripts/check-pii-text.mjs` over the commit message (`.husky/commit-msg`) and over the PR title and body (CI), and `scripts/claude-pii-hook.mjs` as a `PreToolUse` hook wired in `.claude/settings.json`. The hook is the one that fires *first* — it inspects what an agent is about to write before the write happens, covering `Write`, `Edit`, and the `Bash` commands that write repo files or publish (`git commit`, `gh pr create`). Reads are deliberately untouched: querying real data to diagnose something is the point, and only the flow back into the repo is blocked. Its three allow-lists (`ALLOWED_DOMAINS`, `ALLOWED_HOSTS`, `ALLOWED_REAL_SHAPED_PHONES`) are the review checkpoint — adding an entry should be a deliberate decision with a stated reason.

  What it cannot see: a person's name, a phone number written without its `+`, prose describing a real account, and anything inside an image. Data pulled from a live tenant is context to read and reason about, not content to paste — restate findings in your own words with placeholder values, and the checks stay a backstop rather than the only defence.
...
- **A thrown error is only translatable if its body carries a `code`.** `packages/dashboard-pages/src/api.ts` reads a top-level `code` off the JSON error body and `i18n/translate-error.ts` looks it up under `errors.<code>`, falling back to the raw English `message`. So `throw new BadRequestException('<module>_invalid: …')` reaches the dashboard untranslated, while `throw new BadRequestException({ message: '<module>_invalid: …', code: '<module>_invalid' })` reaches it translated (pattern: `control/cms-drafts.controller.ts`). Add the `code` **and** an `errors.<code>` entry to both `messages/en.json` and `messages/nb.json` only when a dashboard user can actually trigger the error — an upload over the size cap, a concurrent edit, a stale option in a select. Agent- and API-facing validation stays a plain string throw; most of the ~25 code-shaped messages in `backend-core` deliberately have no i18n key. Either way keep the `<code>: …` prefix in the message so agents and direct API callers get a machine-readable string.
- Tests live alongside source (`*.test.ts`, `*.integration.test.ts`). Integration tests gate on `TEST_DATABASE_URL`.
- **React component tests are `*.test.tsx` in `packages/dashboard-pages`.** Its `vitest.config.ts` runs two projects: `logic` (`*.test.ts`, node environment) and `components` (`*.test.tsx`, happy-dom) — the split exists because `setup-gate.test.ts` and `type-floor.test.ts` break under a DOM global. `src/test/render.tsx` wraps a component in `StrictMode` + `NextIntlClientProvider` (the real `messages/en.json`) + `ConfirmDialogProvider`; `src/test/inbox-fixtures.ts` builds `ConversationDetail` / `QueueItemDto` and a full `QueueController` stub. StrictMode is deliberate: double-invoked effects are what turn a render-time state mirror (`replyRef.current = reply` in `conversation-pane.tsx`) into a test failure instead of a customer-visible one. Reach for a component test only where commit ordering or effect interaction *is* the logic — presentational `.tsx` stays untested on purpose, and anything expressible as a pure helper still belongs in a helper with a `*.test.ts`.
- **A `loading.tsx` above a page that calls `notFound()` turns its 404 into a 200.** Rendering a Suspense fallback commits the response to `200 OK` before the page resolves, so a `notFound()` fired below the boundary can no longer set a status — Next only injects `<meta name="robots" content="noindex">` into the already-streaming HTML. `apps/web/app/[locale]/loading.tsx` did this to `/docs/skills/[...slug]`, which answered 200 with "This page could not be found" until it was removed; the trees that actually need a route-transition fallback keep their own (`(auth)/`, `dashboard/`, `dashboard/settings/`), and the segments left under the root boundary render synchronously or are client components with their own spinner. Before adding a `loading.tsx`, check that nothing under it calls `notFound()`; before adding a `notFound()`, check for a `loading.tsx` above it. `apps/web/e2e/not-found.spec.ts` guards the docs-skills case.
