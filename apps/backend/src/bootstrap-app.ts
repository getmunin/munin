...
import { STORAGE } from './common/storage/storage.module.js';
 ...
 *     `publicUrlFor()`.
 *
 * Used by main.ts at boot AND by every integration test that calls
 * `NestFactory.create(AppModule)` — keeps the two in sync so tests
 * actually exercise the production app shape.
 */
export async function createApp(opts: NestApplicationOptions = {}): Promise<INestApplication> {
