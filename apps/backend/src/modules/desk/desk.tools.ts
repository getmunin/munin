...
import { CHANNEL_TYPES, DeskService, STATUSES } from './desk.service.js';
...
@Injectable()
export class DeskAdminTools {
  constructor(@Inject(DeskService) private readonly desk: DeskService) {}

  @McpTool({
    name: 'desk_list_conversations',
    description:
      'List helpdesk conversations for your org, newest activity first. Filter by status (open / snoozed / closed / spam), assignee, or topic.',
    audiences: ['admin'],
    scopes: ['desk:read'],
    input: ListConversationsInput,
  })
  listConversations(args: z.infer<typeof ListConversationsInput>) {
    return this.desk.listConversations(args);
  }

  @McpTool({
    name: 'desk_get_conversation',
    description: 'Read one helpdesk conversation including every public + internal message.',
    audiences: ['admin'],
    scopes: ['desk:read'],
  ...
  assign(args: z.infer<typeof AssignInput>) {
    return this.desk.assignConversation(args);
  }
  ...
  changeStatus(args: z.infer<typeof ChangeStatusInput>) {
    return this.desk.changeStatus(args);
  }

  @McpTool({
    name: 'desk_search_messages',
    description:
      'Substring search over message bodies. Returns the matching messages newest first; use desk_get_conversation to load surrounding context.',
    audiences: ['admin'],
    scopes: ['desk:read'],
    input: SearchInput,
  })
  search(args: z.infer<typeof SearchInput>) {
    return this.desk.searchMessages(args);
  }

  @McpTool({
    name: 'desk_list_channels',
    description: 'List helpdesk channels (email, voice, chat, sms) configured for your org.',
    audiences: ['admin'],
    scopes: ['desk:read'],
    input: EmptyInput,
  })
  listChannels() {
    return this.desk.listChannels();
  }

  @McpTool({
    name: 'desk_create_channel',
    description:
      'Add a new helpdesk channel. Type is one of email / voice / chat / sms. Channel-specific configuration goes in `config`.',
    audiences: ['admin'],
    scopes: ['desk:write'],
    input: CreateChannelInput,
  })
  createChannel(args: z.infer<typeof CreateChannelInput>) {
    return this.desk.createChannel(args);
  }

  @McpTool({
    name: 'desk_list_topics',
  ...
  listTopics() {
    return this.desk.listTopics();
  }

  @McpTool({
    name: 'desk_create_topic',
  ...
  createTopic(args: z.infer<typeof CreateTopicInput>) {
    return this.desk.createTopic(args);
  }
}
