...
describe('createConversationHandler', () => {
  ...
  it('draft-request briefs the agent for the reviewer and hides the handover tool', async () => {
    const seen: Array<{ tools: unknown; config: { systemPrompt: string; volatileSystemPrompt?: string } }> = [];
    ...
    expect(agentCall.config.volatileSystemPrompt).toContain('[Draft request]');
    expect(agentCall.config.systemPrompt).not.toContain('[Draft request]');
  ...
  it('the audit sees the thread, so a short reaction is judged in context', async () => {
            ...
            messages: [
              {
                id: 'msg_1',
                authorType: 'end_user',
                body: 'Er Globex virkelig gratis?',
    ...
    expect(threadSection).toContain('customer: Er Globex virkelig gratis?');
  ...
  it('parks the reply for review when the send is rejected because the topic turned draft_only', async () => {
    const rest = buildRest({
      getConversation: vi.fn(() => Promise.resolve(buildConversation({ channelType: 'email' }))),
    });
    const postSpy = vi.fn(() =>
      Promise.reject(
        new MuninRestError(
  ...
  it('parks the reply on a coded in-process rejection too (backend runner bypasses the REST client)', async () => {
    const rest = buildRest({
      getConversation: vi.fn(() => Promise.resolve(buildConversation({ channelType: 'email' }))),
    });
    const inProcessError = Object.assign(
      new Error(
        'agent_send_not_auto: conversation conv_1 resolves to draft_only, so an agent reply must be parked for review instead of sent',
      ),
      { code: 'agent_send_not_auto' },
    );
