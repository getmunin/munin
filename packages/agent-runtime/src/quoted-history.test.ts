      ...
      quotedThread: [
        {
          ...
          to: 'Anne Britt <psykolog@anneselvik.no>',
...
describe('renderQuotedHistory', () => {
  it('labels the block as quoted so the model does not read it as the customer speaking', () => {
    const rendered = renderQuotedHistory([
      { from: 'uScore <support@uscore.no>', date: '15. september', subject: 'Betalingsrisiko', body: 'Sjekk tallet ditt' },
    ]);
    expect(rendered).toContain('not what the customer wrote');
    expect(rendered).toContain('<quoted_history>');
    expect(rendered).toContain('From: uScore <support@uscore.no>');
    expect(rendered).toContain('Date: 15. september');
    expect(rendered).toContain('Subject: Betalingsrisiko');
    expect(rendered).toContain('Sjekk tallet ditt');
  });
...
describe('summarizeQuotedHistory', () => {
  it('flattens the quote to one attributed line for the audit thread', () => {
    const summary = summarizeQuotedHistory([
      { from: 'uScore', date: '15. september', subject: null, body: 'line one\n\nline two' },
    ]);
    expect(summary).toBe(
      '(quoted email the customer replied to, not their words) uScore · 15. september: line one line two',
    );
  });
