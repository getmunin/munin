...
class ImportConvBody extends createZodDto(
  z.object({
    records: z.object({
      channels: z.array(
        z.object({
...
export class ConvTransferController {
  ...
  @Post('import')
  @HttpCode(200)
  async import(@Body() input: ImportConvBody): Promise<ImportResult> {
    return this.conv.importConv(input.records, input.idMap);
  }
}
