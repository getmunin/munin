...
class ProgressBody extends createZodDto(
  z.object({
    progress: z.object({
...
export class CuratorJobsController {
  ...
  @Post(':id/acknowledge')
  @HttpCode(200)
  async acknowledge(@Param('id') id: string, @Body() input: AckJobBody): Promise<CuratorJobDto> {
    if (input.totalTokens != null) getCurrentContext().aiTokens = input.totalTokens;
  ...
  @Post(':id/progress')
  @HttpCode(204)
  async progress(@Param('id') id: string, @Body() input: ProgressBody): Promise<void> {
    await this.service.updateProgress({ id, progress: input.progress });
  }
}
