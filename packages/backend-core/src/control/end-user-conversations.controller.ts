...
class StartConversationBody extends createZodDto(
  z.object({
...
class ReplyMessageBody extends createZodDto(
  z.object({
    body: z.string().min(1).max(50_000),
  }),
) {}
...
export class EndUserConversationsController {
  ...
  @Post()
  @HttpCode(201)
  async start(@Body() input: StartConversationBody): Promise<ConversationDetail> {
  ...
  @Post(':id/messages')
  @HttpCode(201)
  async reply(@Param('id') id: string, @Body() input: ReplyMessageBody): Promise<MessageDto> {
    const actor = this.requireEndUserActor();
    return translate(() =>
