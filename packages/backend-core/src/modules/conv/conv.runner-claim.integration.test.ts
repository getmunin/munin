...
(skipReason ? describe.skip : describe)('conv runner-claim ownership', () => {
  ...
  it('first runner claims; second runner is rejected', async () => {
    const first = await call(`/api/conversations/${convId}/runner-claim`, {
      method: 'POST',
    ...
    const second = await call(`/api/conversations/${convId}/runner-claim`, {
      method: 'POST',
  ...
  it('current holder can refresh its own lease', async () => {
    await call(`/api/conversations/${convId}/runner-claim`, {
      method: 'POST',
      body: { holder: 'sidecar-A', leaseSeconds: 60 },
    });
    const refresh = await call(`/api/conversations/${convId}/runner-claim`, {
      method: 'POST',
      body: { holder: 'sidecar-A', leaseSeconds: 3600 },
    });
  ...
  it('expired lease can be taken over by another holder', async () => {
    ...
    const takeover = await call(`/api/conversations/${convId}/runner-claim`, {
      method: 'POST',
      body: { holder: 'sidecar-B', leaseSeconds: 3600 },
    });
  ...
  it('release works only for the current holder', async () => {
    ...
    const wrongRelease = await call(`/api/conversations/${convId}/runner-release`, {
      method: 'POST',
      body: { holder: 'sidecar-B' },
    });
    expect((wrongRelease.body as { released: boolean }).released).toBe(false);

    const correctRelease = await call(`/api/conversations/${convId}/runner-release`, {
      method: 'POST',
  ...
  it('closing a conversation auto-clears the runner claim', async () => {
    ...
    const close = await call(`/api/conversations/${convId}/status`, {
      method: 'POST',
      body: { status: 'closed' },
    });
  ...
  it('agent post with stale sinceMessageId is rejected as race', async () => {
    ...
    const racePost = await call(`/api/conversations/${convId}/messages`, {
      method: 'POST',
