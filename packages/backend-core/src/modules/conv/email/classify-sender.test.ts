...
describe('classifySender', () => {
  ...
  it('suppresses an out-of-office auto-reply', () => {
    const c = classifySender(
      h({ From: 'ola.nordmann@example.no', 'Auto-Submitted': 'auto-replied' }),
  ...
  it('suppresses a bounce carrying X-Failed-Recipients', () => {
    const c = classifySender(
      h({ From: 'noreply@relay.acme.com', 'X-Failed-Recipients': 'ola.nordmann@initech.test' }),
      'noreply@relay.acme.com',
    );
    expect(suppressionReason(c)).toBe('bounce');
  });
  ...
  it('suppresses a sender whose address says it takes no replies', () => {
    for (const from of [
      'noreply.autoresponder.no@contoso.test',
