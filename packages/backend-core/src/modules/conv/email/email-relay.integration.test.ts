  ...
  () => {
    ...
    it('attributes an Outlook reply to its sender, not to the support address it quotes', async () => {
      const res = await postRelay({
        ...
        raw: Buffer.from(
          [
            'From: Ola Nordmann <ola@kunde.test>',
            `To: <${relayAddress}>`,
      ...
      expect(emails).toContain('ola@kunde.test');
    ...
    function rawEspBounce(): string {
      ...
      return [
        ...
        'Content-Type: text/plain; charset="utf-8"',
        '',
        "Your message to ola.nordmann@initech.test was rejected: the recipient's mailbox is full.",
        '',
        `X-HE-Meta: ${blob}`,
    ...
    it('files a Microsoft 365 out-of-office away closed, though its envelope sender is postmaster@', async () => {
      const raw = [
        'From: Ola Nordmann <ola.nordmann@fabrikam.test>',
        `To: <${relayAddress}>`,
        'Return-Path: <postmaster@example-tenant.outbound.protection.outlook.test>',
        'Subject: Automatic reply: Nyhetsbrev februar',
        'Message-ID: <ooo-m365@fabrikam.test>',
        'Content-Type: text/plain; charset="utf-8"',
        '',
    ...
    it('files away closed a deleted-mailbox notice from an address that takes no replies', async () => {
      const raw = [
        ...
        'Message-ID: <deleted-mailbox@contoso.test>',
        'Content-Type: text/plain; charset="utf-8"',
        '',
    ...
    it('still answers a real question sent as a reply to the same newsletter', async () => {
      ...
      const real = [
        ...
        'In-Reply-To: <newsletter-feb@marketing.globex.test>',
        'References: <newsletter-feb@marketing.globex.test>',
        'Content-Type: text/plain; charset="utf-8"',
        '',
    ...
    it('records the recipient a delivery-status report names as undeliverable', async () => {
      const raw = [
        'From: Mail Delivery System <MAILER-DAEMON@mx.contoso.test>',
        `To: <${relayAddress}>`,
        'Subject: Undelivered Mail Returned to Sender',
        'Message-ID: <dsn-945@mx.contoso.test>',
        'Content-Type: multipart/report; report-type=delivery-status; boundary="dsnsep"',
        '',
        '--dsnsep',
        'Content-Type: text/plain; charset="us-ascii"',
        '',
        'This is the mail system at host mx.contoso.test.',
        '',
        '--dsnsep',
        'Content-Type: message/delivery-status',
        '',
        'Reporting-MTA: dns; mx.contoso.test',
        '',
        'Final-Recipient: rfc822; Gone.Person@Contoso.test',
        'Action: failed',
        'Status: 5.1.1',
      ...
      const rows = await db
        ...
        .where(
          and(
            ...
            eq(schema.crmAddressDeliverability.address, 'gone.person@contoso.test'),
          ),
        );
      expect(rows).toHaveLength(1);
    ...
    it('soft-marks the address a no-reply mailbox notice answers on an existing thread', async () => {
      ...
      const [contact] = await db
        ...
        .values({ orgId, name: 'Gone Colleague', email: 'Gone.Colleague@Contoso.test' })
        .returning();
      const [conv] = await db
      ...
      const raw = [
        ...
        `To: <${relayAddress}>`,
        'Subject: This mailbox is no longer available',
        'Message-ID: <no-reply-945@contoso.test>',
      ...
      const rows = await db
        ...
        .where(
          and(
            eq(schema.crmAddressDeliverability.orgId, orgId),
            eq(schema.crmAddressDeliverability.address, 'gone.colleague@contoso.test'),
          ),
        );
      expect(rows).toHaveLength(1);
