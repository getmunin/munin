...
(skipReason ? describe.skip : describe)('Email channel integration: SMTP send + IMAP poll + threading', () => {
  ...
  it('an out-of-office reply lands in the thread but raises no attention and starts no jobs', async () => {
    ...
    try {
      ...
      fetcher.push(rfc822({
        from: 'Ola Nordmann <ola@ooo.test>',
        to: 'support@acme.test',
        subject: 'Automatic reply: Question about pricing',
        messageId: 'ooo-auto@ooo.test',
        extraHeaders: ['Auto-Submitted: auto-replied'],
        body: 'I am out of the office until Friday 11/9-26. If any urgency please call +47 99999999.',
      }));
      expect((await inboundWorker.tick()).messagesIngested).toBe(1);

      const conv = await convFor('ola@ooo.test');
