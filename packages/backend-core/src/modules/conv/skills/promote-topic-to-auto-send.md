...
To step back to review, set `"mode": "draft_only"`; to silence the agent in a topic, `"mode": "off"`; to remove the override entirely, `"mode": null`. A demotion takes effect immediately, including for replies already being composed — an agent send that lands after it is refused and parked as a draft, so you never have to wait out in-flight turns. Watch `conv_list_topic_automation` after a promotion — if `rejected` or `edited` start climbing on the residual reviewed replies, or customers start re-asking, demote and let the review record rebuild.
