...
- **Auto-replies and bounces**: an inbound message whose headers mark it machine-generated (RFC 3834 `Auto-Submitted`, `X-Autoreply`, `X-Autorespond`, `Precedence: junk`, `X-Auto-Response-Suppress`, or an empty/`mailer-daemon` `Return-Path`) is stored on the thread with `metadata.suppressed` set to `auto_reply` or `bounce`, and nothing else happens: no agent turn, no draft, no human-attention flag, no topic pass, and a closed conversation stays closed. Out-of-office notices therefore stop reaching the operator queue while still being readable in the thread, and they do not count as the prospect having replied to an outreach sequence. There is no toggle — the classification is header-based and always on.
- **Mail loops**: Munin marks every reply its agent sends with RFC 3834 `Auto-Submitted: auto-replied`, and drops an inbound message whose `Message-ID` matches one it sent itself. Both matter when the polled mailbox also receives the channel's own outbound — a `hello@` alias delivering into the `mailmaster@` mailbox it is sent from, say — because without them each reply is ingested as a fresh customer email and answered again, one round per poll. Replies an operator types stay unmarked; they are not automatic responses, and the `Message-ID` check is what covers them. Relatedly, `conv_send_email_channel_test` refuses a `to` equal to the channel's own address (`conv_test_self_addressed`): send the test to a mailbox you can read instead.
