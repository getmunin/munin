...
4. `conv_test_channel` re-verifies stored credentials any time; on an SMS channel `conv_send_channel_test_message` also sends a real message.
...
- SMS: `conv_send_channel_test_message { channelId, to }` sends a real message end-to-end. Voice vendors have no test send — the tool answers `channel vendor 'vapi' does not support test sends`.
