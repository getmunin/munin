...
const KB_SWEEP_PROMPT =
  'Run a KB curation pass over the last 7 days of resolved-handover conversations. Follow the procedure in the skill exactly. Scope the pass with conv_list_conversations({ handover: "resolved", since: <7 days ago> }) — do not widen it. Skip duplicates and one-off answers. File each candidate via kb_propose_curation_candidate. Stop when there are no more candidates to file.';
  ...
  onModuleInit(): void {
    ...
    const sweeps: SweepDef[] = [
      ...
      },
      {
        ...
        cron: parseEnvCron({
          name: 'MUNIN_CURATOR_OUTREACH_FIRST_TOUCH_CRON',
          default: CronExpression.EVERY_WEEK,
        }),
        jobUri: 'skill://outreach/draft-first-touch-email',
        userPrompt: OUTREACH_FIRST_TOUCH_PROMPT,
        dedupeKey: 'outreach-first-touch:scheduled',
      },
      {
