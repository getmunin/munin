...
This is fundamentally different from `skill://outreach/draft-first-touch-email`:
...
- The proposal lands in `pending` status, visible to the operator on `/dashboard/review` (Waiting tab) with a Reply badge.
