...
description: Operator review pass over drafted outreach proposals — approve each pending draft (sending it now or scheduling it for a named time), dismiss it, or call off a scheduled send, plus the two agent-side corrections, revise and withdraw. Voice, SMS and public thread comments are approved only in the Munin dashboard. In MCP App hosts this renders the interactive Munin Inspector panel.
audiences: [admin]
---

# Review pending outreach proposals

Every outbound message in Munin ships through a human-approved gate: curators file drafts as **pending proposals** (`skill://outreach/draft-first-touch-email`, `skill://outreach/draft-first-touch-sms`, `skill://outreach/draft-first-touch-call`, `skill://outreach/draft-reply-email`, `skill://outreach/draft-followup-email`, `skill://outreach/draft-thread-comment`), and nothing leaves the org until an operator — or an admin agent acting on their explicit instruction — decides each one. This skill is that decision pass.

**Calls, text messages and public thread comments are approved in the dashboard, never here.** A proposal whose campaign runs on a voice or SMS channel, and every `thread_comment` proposal, can only be approved by a signed-in person in the Munin dashboard. `outreach_approve_proposal` refuses every other caller — an agent, an admin API key, the Slack button — with `outreach_invalid: … approved by a signed-in person in the Munin dashboard`. That is the safety floor for outbound calling and for public speech, not a configuration you can route around: don't retry, don't look for another tool, and don't ask for a credential that would work. Present the draft, say it is waiting for someone to place the call or post the comment from the dashboard inbox, and stop. You can still `outreach_revise_proposal`, `outreach_withdraw_proposal`, and `outreach_dismiss_proposal` on these — none of them send anything.

**Approving is the send decision.** `outreach_approve_proposal` is not a status flip: for an `initial` proposal it creates the outbound conversation and sends the first email through the campaign's channel (appending the CTA link and unsubscribe footer per campaign settings); for a `reply` or `followup` it sends the draft verbatim on the existing conversation; for a `thread_comment` it opens a conversation for the thread and posts the comment publicly under the org's forum account. It happens the moment you approve — unless a future send time applies, in which case the proposal parks at `status: "approved"` and a worker delivers it at that time (see **Scheduling a send** below). Once a message is out there is no undo. Never approve in bulk without reading each draft.
...
**A `thread_comment` has no recipient — read its `target` instead.** There is no contact, no consent record and no unsubscribe footer, because the target is a public thread rather than a person. What the reviewer needs to see is the subreddit, the thread title, the permalink, and the rule check the curator recorded in `evidence`. Two limits are re-checked at approval and can refuse a draft that was fine when filed: the campaign already has a comment on that thread, or the campaign is at its `maxPerWeekPerSubreddit` / `maxCommentsPerDay` cadence cap. Both refusals are answers, not obstacles.
...
|---|---|---|
| `outreach_approve_proposal` | operator | Send it — now, or at a named time. Email only — voice, SMS and thread comments are dashboard-only. |
...
Email proposals get **Approve & send** and **Dismiss** buttons — the approve button reads **Approve & schedule** on a draft carrying a future `proposedSendAt`, with the time spelled out above it, so the operator can see they are authorizing a later send rather than an immediate one. Voice, SMS and thread-comment proposals get **Dismiss** only, plus a line pointing the operator at the dashboard — the panel cannot place a call or post a comment, because the server refuses any approval that does not come from a signed-in dashboard session.
...
2. **Read `delivery` first.** It names the channel and the exact destination. If `delivery.channelType` is `voice` or `sms`, or the proposal's `kind` is `thread_comment`, there is nothing for you to approve: present the draft and the destination — the phone number, or the thread permalink — tell the operator it is waiting in the dashboard inbox, and move on. If `delivery.destination` is `null`, the contact has no address or number on file and approval would fail — say so instead of trying. On a `thread_comment` the destination is the thread permalink and both `appendsCta` and `appendsUnsubscribe` are always false: Munin adds nothing to a public comment.
...
- **Don't try to place a call or post a comment.** If a voice, SMS or thread-comment approval is refused, that refusal is the product working as designed. There is no tool, key, or argument that makes it succeed — calls, texts and public comments leave Munin only when a person clicks in the dashboard.
