...
const SetRoutingInputSchema = z.object({
  ...
  purpose: z
    .enum(['default', 'escalations', 'approvals', 'content'])
    .optional()
    .describe(
      "'default' (all mirrored conversations; required before mirroring starts), 'escalations' (handover alerts; falls back to the default channel when unset), 'approvals' (approval-queue notifications: CRM merge proposals, outreach drafts, KB curation candidates; falls back to escalations, then default), or 'content' (CMS publish announcements; falls back to default)",
    ),
  mention: z
    .string()
    .max(64)
