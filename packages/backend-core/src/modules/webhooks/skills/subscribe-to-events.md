...
- **Conv** — `conversation.created`, `conversation.subject_changed`, `conversation.status_changed`, `conversation.assigned`, `conversation.released`, `conversation.taken_over`, `conversation.agent_mode_changed`, `conversation.topic_automation_changed`, `conversation.handover_requested`, `conversation.handover_resolved`, `conversation.greet_requested`, `conversation.draft_requested`, `conversation.draft_ready`, `conversation.note_added`, `conversation.message.received`, `conversation.message.sent`, `conversation.message.body_revised`, `conversation.message.opened`, `conversation.message.read`, `conversation.voice.call_ended`
...
The `cms.entry.published` payload also carries `title`, `url` (the collection's `settings.liveUrl` template resolved for this entry's locale, `null` when unset), and `previousStatus` (`"draft"`, `"scheduled"`, `"published"` for a no-op republish, or `null` when the entry was created published) — enough to revalidate exactly one path, or skip work when nothing actually changed status. Every `cms.entry.*` payload also carries `translationGroupId`, the id shared by every locale variant of one article — revalidate the whole language switcher off it, or group the variants in your own UI.
