...
- 4bab452: Give unrouted URLs a Munin 404 instead of the stock Next.js one
...
- 22c466b: Export the settings scaffold so an embedder can match the settings design.
...
- fd84e97: Align the settings sidebar's active-item highlight with the console sidebar: the cobalt bar and paper fill now bleed to both edges instead of sitting inset, and item padding matches.
...
- 46005da: Drop the raw model id from a picker option that already has a name.
...
- 8ddf18f: Show the sender's email address beside their name in the conversation thread pane
  ...
  `Nordmann, Ola <ola.nordmann@example.no>`, and falls to its own line when the conversation has no subject
...
- 411b50f: Format phone numbers on the end-user list the same way the rest of the dashboard does — an end user identified only by phone now shows `+47 12 34 56 78` instead of the raw E.164 string.
