...
- d8a22c3: Schema groundwork for publishing as a Facebook Page.
  ...
  hands back a long-lived _user_ token, which is only a key to the list of Pages that person
...
- 2bd3138: Set the RLS bypass from inside `conv_next_display_id` instead of attaching it to the function, so a non-superuser migration role can create it.
...
- 7def0de: Repair mail that a forwarding hop got misclassified as an auto-reply.
