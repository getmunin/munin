...
- 2d75df3: fix(docs-pages): resolve dashboard links against `NEXT_PUBLIC_APP_URL` instead of assuming same-origin

  The six links into `/dashboard/settings/api-keys` — the "Get a key →" button in the docs topbar plus the inline "Settings → API keys" links in the five `connect-*` guides — used the next-intl `Link`, which localizes the href against the _current_ app. In OSS that is correct: `apps/web` serves `/[locale]/docs` and `/[locale]/dashboard` from one origin. On the cloud marketing site it is not — there is no dashboard route there, so every one of the 66 built docs pages carried a dead `/en/dashboard/settings/api-keys`, the topbar button included.
